...
export default function FloatingLines({
  ...
  useEffect(() => {
    ...
    // gradient setup
    const stops = linesGradient && linesGradient.length > 0
      ? linesGradient.slice(0, MAX_GRADIENT_STOPS)
      : (() => {
          const css = getComputedStyle(document.documentElement)
          const accent = css.getPropertyValue('--accent').trim() || '#22D3EE'
          const accentSoft = css.getPropertyValue('--accent-soft').trim() || '#0EE2E0'
          const dimLine = '#0D0D0D' // fallback very dim
          return [accent, accentSoft, dimLine]
        })()
